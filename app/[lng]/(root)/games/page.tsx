...
import FillterData, {
  type PlatformFilter,
  type SortFilter,
} from "@/components/fillters/fillter-data";
...
export default function GamesPage() {
  ...
  const handlePlatformSelect = useCallback((filter: PlatformFilter) => {
    setPlatformFilter(filter);
  }, []);

  const handleSortSelect = useCallback((filter: SortFilter) => {
  ...
  const filteredGames = useMemo(() => {
    ...
    const result = games.filter((game) => {
      ...
      const title =
        ...
        (game.langData
          ? Object.values(game.langData).find((item) => item?.title)?.title
          : "") ||
        "";
      ...
      const matchesSearch =
        query.length === 0 || title.toLowerCase().includes(query);
      ...
      if (platformFilter === "ios") {
        const hasIOS =
          game.selectedOS?.ios === true || game.selectedOS?.macos === true;
    ...
    return result;
  }, [games, platformFilter, sortFilter, searchQuery, currentLng]);
  ...
  return (
    <main
      className={cn(
        "relative min-h-screen pt-[20vh] pb-16 px-4 md:px-8 lg:px-12 transition-colors duration-500",
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        {loading && (
          <div className="py-20 text-center space-y-3">
            <div className="inline-block size-8 border-[3px] border-blue-600 border-t-transparent rounded-full animate-spin" />
          ...
          <div className="py-20 text-center opacity-60 space-y-2">
            ...
            {searchQuery && (
              <p className="text-sm text-slate-500">
                &quot;{searchQuery}&quot; bo‘yicha o‘yin topilmadi.
              </p>
            )}
          </div>
        )}
              ...
              {filteredGames.map((game) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={cn(
                        "relative p-5 rounded-[2rem] border backdrop-blur-xl transition-all duration-300",
                        "hover:-translate-y-1.5 h-full flex flex-col justify-between overflow-hidden",
