...
export default function MyNewsPage() {
  ...
  // Bazadan malumotlarni yuklab olish
  ...
  return (
    <div className="w-full max-w-[1100px] mx-auto my-20 space-y-8 px-3 sm:px-4 lg:px-0 animate-in fade-in duration-500">
      {/* HEADER PART */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <Newspaper className="text-blue-600 shrink-0" size={28} />
            <span className="truncate">Yangiliklarim</span>
          </h1>
              ...
              >
                ...
                <div className="relative aspect-video w-full bg-slate-100 dark:bg-slate-950 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 flex items-center gap-1.5 max-w-[calc(100%-1.5rem)]">
                    {status === "requested" && (
                      <div className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 shadow-sm backdrop-blur-md text-white bg-amber-500/90 whitespace-nowrap">
                        <Clock size={12} />
                        Kutilmoqda
                      </div>
                    )}
                    ...
                    {status === "rejected" && (
                      <div className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 shadow-sm backdrop-blur-md text-white bg-red-500/90 whitespace-nowrap">
                        <XCircle size={12} />
                        Rad etilgan
                      </div>
                    )}
                    ...
                    {status === "approved" && (
                      <div
                        className={cn(
                          "px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 shadow-sm backdrop-blur-md text-white whitespace-nowrap",
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4 min-w-0">
                  <div className="space-y-3 min-w-0">
                    <div className="flex items-center justify-between gap-3 text-[11px] font-bold text-slate-400 dark:text-slate-500 min-w-0">
                      <span className="flex items-center gap-1 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded-md text-slate-600 dark:text-slate-400 min-w-0 max-w-[60%]">
                        <Layers size={12} className="shrink-0" />

                        <span className="truncate">
                          {item.selectedGame || "Umumiy"}
                        </span>
                      </span>
